    ...
    static int play(){
        ...
        while (!Quitter) {
            ...
            while (NbPoints != 0 && !Mort && !Quitter) {
                Compteur2++;
                if (Compteur2 > 8) {
                    //calcul du fantome le plus proche
                    heuristic[0] = pow(PacMan.GetX() - Bashful.GetX(), 2) + pow(PacMan.GetY() - Bashful.GetY(), 2);
                    heuristic[1] = pow(PacMan.GetX() - Pokey.GetX(), 2) + pow(PacMan.GetY() - Pokey.GetY(), 2);
                    heuristic[2] = pow(PacMan.GetX() - Shadow.GetX(), 2) + pow(PacMan.GetY() - Shadow.GetY(), 2);
                    heuristic[3] = pow(PacMan.GetX() - Speedy.GetX(), 2) + pow(PacMan.GetY() - Speedy.GetY(), 2);
                    int a = heuristic[3];
                    int b = 3;
                    for (int i = 0; i < 3; i++) {
                        if (heuristic[i] < a) {
                    ...
                    //aller dans la direction opposé du fantome le plus proche
                    //si c'est bashful
                    if (b = 0) {
                        int c, d;
                        c = PacMan.GetX() - Bashful.GetX();
                        d = PacMan.GetY() - Bashful.GetY();
                        if (abs(c) < abs(d) && c != 0) {
                            if (c < 0) {
                                PacMan.Bouger(EVDroite, Miroir);
                            } else { PacMan.Bouger(EVGauche, Miroir); }
                        } else {
                            if (d < 0) {
                                PacMan.Bouger(EVHaut, Miroir);
                            } else { PacMan.Bouger(EVBas, Miroir); }
                        }
                    }
                    //si c'est pokey
                    if (b = 1) {
                        int c, d;
                        c = PacMan.GetX() - Pokey.GetX();
                        d = PacMan.GetY() - Pokey.GetY();
                        if (abs(c) < abs(d) && c != 0) {
                            if (c < 0) {
                                PacMan.Bouger(EVDroite, Miroir);
                            } else { PacMan.Bouger(EVGauche, Miroir); }
                    ...
                    if (b = 0) {
                        int c, d;
                        c = PacMan.GetX() - Shadow.GetX();
                        d = PacMan.GetY() - Shadow.GetY();
                        if (abs(c) < abs(d) && c != 0) {
                            if (c < 0) {
                                PacMan.Bouger(EVDroite, Miroir);
                            } else { PacMan.Bouger(EVGauche, Miroir); }
                    ...
                    if (b = 0) {
                        int c, d;
                        c = PacMan.GetX() - Speedy.GetX();
                        d = PacMan.GetY() - Speedy.GetY();
                        if (abs(c) < abs(d) && c != 0) {
                            if (c < 0) {
                                PacMan.Bouger(EVDroite, Miroir);
                            } else { PacMan.Bouger(EVGauche, Miroir); }
                ...
                DessinerRectangle(NbPixelsParCase * Largeur - MurMince, NbPixelsParCase * 9, MurMince, NbPixelsParCase,
                                  255, 255, 255);
                DessinerRectangle(NbPixelsParCase * 9, NbPixelsParCase * 8 + MurMince, NbPixelsParCase, MurMince, 255,
                                  255, 255);
                ...
                if (Fuite == 0 || Fuite == 4 || Fuite == 5 || Fuite == 8 || Fuite == 9 || Fuite == 12 || Fuite == 13 ||
                    Fuite == 16 || Fuite ==
                                   17) {   //On les fait flacher avant de redevenir normal (pour pas avoir de surprise, c'est chien)
